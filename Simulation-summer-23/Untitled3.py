...
from multiprocessing import Process, Pipe  # BD: added Pipe for proper piping of data between processes
...
class LivePlotter:
    ...
    def __init__(self, delta):
        ...
        self.root.wm_title("Arm Angle Graph")
        self.fig = plt.figure()
    ...
    def update(self, _, sl_master : dict):
        ...
        print("updated points and lines")
        for t in plt.gcf().texts:
            plt.gcf().texts.remove(t)
           # t.set_visible(False)
        wrist = np.array([sl_master["wrist"][0], sl_master["wrist"][1]])
        elbow = np.array([sl_master["elbow"][0], sl_master["elbow"][1]])
        shoulder=np.array([sl_master["shoulder"][0], sl_master["shoulder"][1]])
       # print(wrist,elbow,shoulder)
        ba = shoulder - elbow #a - b
        bc = wrist - elbow #c - b
        cosine_angle = np.dot(ba, bc) / (np.linalg.norm(ba) * np.linalg.norm(bc))
        angle = np.arccos(cosine_angle)
        if angle==angle:  #check for NaN
            plt.figtext(0.1, 0.1,round(np.degrees(angle)))
        self.canvas.draw()
        print("drew")
        self.root.update_idletasks()
        print('updated idle')
      #  self.root.update()   #never returns?
        print('redraw')
...
def plot_func(sl_master : dict, delta, pipe):
    # BD: update sl_master via pipe
    sl_master = pipe.recv()
...
def simulate(sl_master : dict, pipe):
    ...
    with mp_pose.Pose(min_detection_confidence=.5, min_tracking_confidence=.5) as pose: #play with args to see what tracking works better
        while capture.isOpened(): 
            ...
            # BD: after updating data, send sl_master to plot_func via pipe:
            pipe.send(sl_master)
...
def update_data(sl_master : dict, pose_landmarks, mp_pose : mp.solutions.pose):
    if(pose_landmarks):
        ...
        #wrist = np.array([pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST.value].x,pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST.value].y,pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST.value].z])
        #print(wrist)
        wrist = np.array([pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST.value].x,pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST.value].y])
        #elbow = np.array([pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ELBOW.value].x, pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ELBOW.value].y, pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ELBOW.value].z])
        elbow = np.array([pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ELBOW.value].x, pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ELBOW.value].y])
       # shoulder = np.array([pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].x, pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].y, pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].z])
        shoulder = np.array([pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].x, pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].y])
        ba = shoulder - elbow #a - b
        bc = wrist - elbow #c - b
...
if __name__ == '__main__':
    # BD: implementing piping to properly share data between p1 and p2
    plot_pipe, sim_pipe = Pipe()
    ...
    p1 = Process(target = plot_func, args = (sl_master, max_delta, plot_pipe, ) )
    p2 = Process(target = simulate, args = (sl_master, sim_pipe, ))
    p1.start()
    p2.start()
