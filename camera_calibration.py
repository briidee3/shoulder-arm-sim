...
#criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)  # criteria for stopping point of cornerSubPix
...
def update_gui(camera, tk_image_label, root, is_countdown, countdown_end_time, is_taking_pic, num_pics, feedback_var, num_var):
    ...
    # call next cycle
    root.after(delay, update_gui, camera, tk_image_label, root, is_countdown, countdown_end_time, is_taking_pic, num_pics, feedback_var, num_var)
...
def run_cam_calib_gui():
    try:
        ...
        num_pics = 5            # denote how many pics to take
        ...
        instructions = '''
            ...
            and take {num_of_pics} pictures of it in different orientations
        ...
        instructs_text.insert(END, instructions)
        ...
        pic_fb_entry = Entry(pic_fb_label, textvariable = pic_fb_var, state = DISABLED)
        ...
        pic_num_entry = Entry(pic_fb_label, textvariable = pic_num_var, state = DISABLED)

        # create booleans and string to help handle counting down
        is_countdown = BooleanVar()
        is_countdown.set(False)
        is_taking_pic = BooleanVar()
        is_taking_pic.set(False)
        countdown_end_time = StringVar()
        countdown_end_time.set("0")
        # create button for taking pictures
        #take_pic_command = partial(take_picture_gui, cam, num_pics, countdown, pic_fb_var, pic_num_var) # put together function call with args = Button(input_frame, text = "Take picture", command = take_pic_command)
        ...
        try:
            # start the GUI
            root.bind("<Escape>", root.destroy)       # use escape key to exit program
            root.after(delay, update_gui, cam, image_label, root, is_countdown, countdown_end_time, is_taking_pic, num_pics, pic_fb_var, pic_num_var)
            root.mainloop()
...
def calibrate_camera(dir_name = "calibrate_pics", camera = cv2.VideoCapture(1), checkerboard = (6,9), criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001), calc_reproj_err = True, save_to_file = True):
    try:
        ...
        objp[0, :, :2] = np.mgrid[0:checkerboard[0], 0::checkerboard[1]].T.reshape(-1, 2)   # creating a mesh grid
        ...
        for pic in pics:
            img = cv2.read(pic)                           # read the current pic into a variable
            ...
            if ret == True:
                ...
                img = cv2.drawChessboardCorners(img, checkerboard, corners_refined, ret)            # overlay found corners onto img
            ...
            prev_img_shape = gray.shape[::-1]   # get rows and cols of img
